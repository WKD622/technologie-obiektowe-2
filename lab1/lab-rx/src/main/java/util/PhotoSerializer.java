...
public class PhotoSerializer {
	...
	private void createLibraryDirectory() throws IOException {
		...
		if (!photoLibraryDir.isDirectory()) {
			throw new IOException("This is not valid photo library directory path or directory could not be created: "
					+ photoLibraryPath);
		}
	}
	...
	private String getPhotoPath(Photo photo) {
		return Paths.get(photoLibraryPath, String.format("%s.%s", photo.getId().toString(), photo.getExtension()))
				.toString();
	}
}
